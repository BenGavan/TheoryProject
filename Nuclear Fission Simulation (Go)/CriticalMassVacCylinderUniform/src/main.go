...
func kForR() {
	r := 0.0005 / 500 * 200
	h := 3.0
	s := newSphere(r, h, 0, Vector{0, 0, 0})
...
func openFile() *os.File {
	filepath := "data/cylindrical-critical-mass-plot-fast.txt"
...
func run() {
	var radius = 0.001 / 500              // meters
	var height = 2.0                   // meters
	var shieldingThickness float64 = 0 // meters
	var numberNeutronsStart = 5000    // Number of neutrons starting

	s := newSphere(radius, height, shieldingThickness, Vector{0, 0, 0})
...
func averageNeutronsProduced(c cylinder, nStartNeutrons int) (float64, float64) {
	...
	for i := 0; i < n; i++ {
		previousFissionLocations := performIteration(&c, nStartNeutrons, fissionLocations)
		neutronsProduced[i] = 2.43 * float64(len(previousFissionLocations)) // TODO: Change for thermal (2.43) / fast (2.5)
...
func performIteration(c *cylinder, nStartNeutrons int, previousFissionLocations []FissionLocation) []Vector {
	...
	//for i := 0; i < nStartNeutrons; i++ {
	//	position := randomPointInSphere(c.radius)
	//	ns[i] = newNeutron(position, c)
	//}
