...
def random_point_in_cylinder(radius, height):
    x = np.random.uniform(-1, 1, 1)[0] * radius
    y = np.random.uniform(-1, 1, 1)[0] * radius
    z = np.random.uniform(-1, 1, 1)[0] * height / 2
    ...
    if r < radius:
        ...
        return random_point_in_cylinder(radius, height)
...
class Neutron(object):
    ...
    def is_in_cylinder(self):
        r2 = pow(self.x, 2) + pow(self.y, 2)
        r = np.sqrt(r2)
        if r < radius and -height / 2 < self.z < height / 2:
            return True
        return False
...
def average_produced(radius):
    ...
    for i in range(generations):
        ...
        for j in range(number_neutrons):
            x, y, z = random_point_in_cylinder(radius, height)
...
def find_critical_radius(radius, height):
    tolerance = 1
    step = 0.01
    ...
    plt.plot(radius_history, average_history)
    plt.title("Cylindrical U-235 in an infinite vacuum (n = {})".format(number_neutrons))
    plt.xlabel("Radius")
    plt.ylabel("Average Neutrons produced")
    plt.savefig("plots/sphere-u235-cm.png")
    plt.show()

    return radius

zs = np.linspace(100, 1000, 10)
...
# 8.484000000000002
# 8.5
# Radius = 8.489999999999995
# Radius = 8.508000000000006
